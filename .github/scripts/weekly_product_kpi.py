...
# Allowlisted, non-sensitive transport error classifications. Only the exception
# TYPE and (for HTTPError) the integer status code are inspected — never the
# response body, reason string, URL, headers, Authorization, API key, query, or
# any exception text / traceback. Unmapped HTTP statuses round to HTTP_OTHER.
_HTTP_STATUS_CODES = {
    400: "HTTP_400",
    401: "HTTP_401",
    403: "HTTP_403",
...
class TransportError(RuntimeError):
    """Raised after retries. `code` is a SAFE allowlisted classification token
    (e.g. HTTP_403 / TIMEOUT); it carries no body, URL, reason, header, or key."""
...
def classify_transport_error(ex):
    """Map a transport exception to a SAFE classification token.

    Inspects ONLY the exception type and, for HTTPError, the integer status
    code. Never reads or returns messages, reasons, bodies, URLs, headers, or
    keys. Unmapped HTTP status codes round to HTTP_OTHER.
    """
    if isinstance(ex, urllib.error.HTTPError):
        try:
            code = int(ex.code)
        except Exception:  # noqa: BLE001 - status code missing / non-int
            return "HTTP_OTHER"
        if code in _HTTP_STATUS_CODES:
            return _HTTP_STATUS_CODES[code]
        if 500 <= code <= 599:
            return "HTTP_5XX"
        return "HTTP_OTHER"
...
def _http_post_json(url, payload_obj, headers):
    ...
    On failure raises TransportError whose `code` is a SAFE classification
    token (no response body, reason, URL, header, or key)."""
    data = json.dumps(payload_obj).encode("utf-8")
    last_code = "UNKNOWN_TRANSPORT_ERROR"
    for _ in range(HTTP_RETRIES + 1):
        try:
            req = urllib.request.Request(url, data=data, headers=headers, method="POST")
            with urllib.request.urlopen(req, timeout=HTTP_TIMEOUT) as resp:
                return resp.read()
        except Exception as ex:  # noqa: BLE001 - classify to a safe token only
            last_code = classify_transport_error(ex)
    raise TransportError(last_code)
...
def main():
    ...
    except TransportError as ex:  # safe HTTP/transport classification only
        print("query failed (%s); sending nothing" % ex.code, file=sys.stderr)
        return 1
    except Exception as ex:  # noqa: BLE001 - schema/parse error (type name only)
    ...
    except Exception as ex:  # noqa: BLE001 - type name only (no message / body)
        print("format/safety check failed (%s); sending nothing" % type(ex).__name__, file=sys.stderr)
        return 1

    if mode == "send":
        webhook = os.environ.get("SLACK_PRODUCT_WEBHOOK_URL", "").strip()
        if not webhook:
